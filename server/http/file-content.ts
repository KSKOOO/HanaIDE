...
export function serveBufferContent(c, {
  buffer,
  filename = "",
  mime = guessMime(filename),
  etag = null,
  ...
  const resolvedEtag = etag || `W/"${size}-${hashBufferForEtag(payload)}"`;
  if (c.req.header("if-none-match") && c.req.header("if-none-match") === resolvedEtag) {
    c.header("ETag", resolvedEtag);
    return c.body(null, 304);
  }
  ...
  return c.body(payload.subarray(start, end + 1), status);
}
...
function hashBufferForEtag(buffer) {
  let hash = 2166136261;

...
describe("macos Cua provider", () => {
  ...
  it.skipIf(!isMacOS)("prefers a Hana-bundled Computer Use helper over an external Cua Driver install", () => {
    const command = resolveCuaDriverCommand({
      env: {
        HANA_ROOT: "/Applications/HanaIDE.app/Contents/Resources/server",
        HANA_CUA_DRIVER_PATH: "/opt/cua-driver",
      },
      existsSync: (p) => p === "/Applications/HanaIDE.app/Contents/Resources/computer-use/macos/hana-computer-use-helper"
        || p === "/opt/cua-driver",
      homeDir: "/Users/hana",
      arch: "arm64",
    ...
    expect(command).toBe("/Applications/HanaIDE.app/Contents/Resources/computer-use/macos/hana-computer-use-helper");
  });

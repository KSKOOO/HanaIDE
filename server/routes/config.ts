...
import { splitByScope } from '../../shared/config-scope.ts';
import {
...
function emitConfigAppEvents(engine: any, { globalFields, agentPartial, providersChanged }: any) {
  ...
  const vscodeWorkbenchChanged = globalFields.some((field) => (
    typeof field?.key === "string" && field.key.startsWith("vscodeWorkbench.")
  ));
  if (vscodeWorkbenchChanged) {
    emitAppEvent(engine, "vscode-workbench-config-changed", {
      vscodeWorkbench: typeof engine.getVscodeWorkbenchPreferences === "function"
        ? engine.getVscodeWorkbenchPreferences()
        : null,
    });
  }
}
...
export function createConfigRoute(engine: any) {
  ...
  route.get("/config", async (c) => {
    try {
      ...
      return c.json(await buildSettingsConfigResponse(engine, {
        kind: "current",
        rawConfig: raw,

...
Beginner mode tone: encouraging, concrete, and guided. Say that the user can describe the feature in plain language, HanaIDE will help turn it into a plugin plan and scaffold, and HanaIDE can answer questions at any step. Ask:

1. 你希望 HanaIDE 多一个什么能力？
...
9. If the user wants publication, choose one channel:
   ...
   - official HanaIDE plugin release: prepare catalog entry and release zip, then run privacy-push before any remote push.
...
- Marketplace metadata lives in the configured HanaIDE plugin catalog, not inside the app repository.
- Official source plugins may live in a separate plugin-catalog repository with a matching catalog entry.
...
- Before pushing a public plugin catalog, run privacy-push and wait for explicit user confirmation.

...
import { isCodingSettingsTab, normalizeSettingsTab } from './settings-tabs';
...
function buildNavItems(pluginSettingsTabs: PluginSettingsTab[]) {
  const tabItems = TAB_ITEMS.filter(item => isCodingSettingsTab(item.id));
...
export function SettingsNav({ onTabChange }: SettingsNavProps) {
  ...
  const normalizedActiveTab = normalizeSettingsTab(activeTab);
  const activeNavTab = normalizedActiveTab === 'plugin-marketplace' ? 'plugins' : normalizedActiveTab;

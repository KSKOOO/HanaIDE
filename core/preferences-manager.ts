...
function isPreferenceRecord(value): value is Record<string, unknown> {
  return !!value && typeof value === "object" && !Array.isArray(value);
}

function normalizeVscodeWorkbenchPreferences(value = {}) {
  const source = isPreferenceRecord(value) ? value : {};
  return {
    enabled: normalizePreferenceBoolean(source.enabled, false),
    galleryEnabled: normalizePreferenceBoolean(source.galleryEnabled, true),
    extensionAutoUpdate: normalizePreferenceBoolean(source.extensionAutoUpdate, false),
    appAutoUpdate: normalizePreferenceBoolean(source.appAutoUpdate, false),
    fallbackToHanaOnFailure: normalizePreferenceBoolean(source.fallbackToHanaOnFailure, true),
  };
}
...
export class PreferencesManager {
  ...
  /** 读取全局出站代理设置。 */
  getVscodeWorkbenchPreferences() {
    return normalizeVscodeWorkbenchPreferences(this._cache.vscode_workbench || {});
  }

  setVscodeWorkbenchPreferences(partial = {}) {
    const prefs = this._mutableCopy();
    prefs.vscode_workbench = normalizeVscodeWorkbenchPreferences({
      ...(prefs.vscode_workbench || {}),
      ...(partial || {}),
    });
    this.savePreferences(prefs);
    return prefs.vscode_workbench;
  }

  getVscodeWorkbenchEnabled() {
    return this.getVscodeWorkbenchPreferences().enabled;
  }

  setVscodeWorkbenchEnabled(enabled) {
    return this.setVscodeWorkbenchPreferences({ enabled }).enabled;
  }

  getVscodeWorkbenchGalleryEnabled() {
    return this.getVscodeWorkbenchPreferences().galleryEnabled;
  }

  setVscodeWorkbenchGalleryEnabled(enabled) {
    return this.setVscodeWorkbenchPreferences({ galleryEnabled: enabled }).galleryEnabled;
  }

  getVscodeWorkbenchExtensionAutoUpdate() {
    return this.getVscodeWorkbenchPreferences().extensionAutoUpdate;
  }

  setVscodeWorkbenchExtensionAutoUpdate(enabled) {
    return this.setVscodeWorkbenchPreferences({ extensionAutoUpdate: enabled }).extensionAutoUpdate;
  }

  getVscodeWorkbenchAppAutoUpdate() {
    return this.getVscodeWorkbenchPreferences().appAutoUpdate;
  }

  setVscodeWorkbenchAppAutoUpdate(enabled) {
    return this.setVscodeWorkbenchPreferences({ appAutoUpdate: enabled }).appAutoUpdate;
  }

  getVscodeWorkbenchFallbackToHanaOnFailure() {
    return this.getVscodeWorkbenchPreferences().fallbackToHanaOnFailure;
  }

  setVscodeWorkbenchFallbackToHanaOnFailure(enabled) {
    return this.setVscodeWorkbenchPreferences({ fallbackToHanaOnFailure: enabled }).fallbackToHanaOnFailure;
  }

...
export function buildSettingsSearchEntries(navItems: SettingsSearchNavItem[]): SettingsSearchEntry[] {
  ...
  const visibleTabIds = new Set(navItems.map(item => item.id));
  const visibleBuiltInEntries = BUILT_IN_SETTINGS_SEARCH_ENTRIES.filter(entry => visibleTabIds.has(entry.tabId));
  const builtInIds = new Set(visibleBuiltInEntries.map(entry => entry.id));
  return [
    ...visibleBuiltInEntries,

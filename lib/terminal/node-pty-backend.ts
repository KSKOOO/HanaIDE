...
import { resolveTerminalShell } from "./shell-resolver.ts";

export async function createAsyncNodePtyBackend() {
  const pty = await import("node-pty");
  ensureUnixSpawnHelperExecutable();
  return {
    spawn({ command = "", profile = "shell", cwd, cols = 80, rows = 24, env, onData, onExit }) {
      const resolved = resolveTerminalShell(command, {
        env: env || process.env,
        profile: shellProfileForTerminalProfile(profile),
      });
      ...
      return {
        ...
        kill: () => proc.kill(),
        dispose: () => disposePtyProcess(proc),
...
function shellProfileForTerminalProfile(profile) {
  const value = String(profile || "shell").trim().toLowerCase();
  if (["powershell", "pwsh", "cmd", "git-bash", "bash", "posix", "default"].includes(value)) {
    return value;
  }
  return "default";
}

function disposePtyProcess(proc) {
  ...
  let taskkill;
  try {
    taskkill = spawnChild("taskkill.exe", ["/PID", String(pid), "/T", "/F"], {
      windowsHide: true,
      stdio: "ignore",
    });
  } catch {

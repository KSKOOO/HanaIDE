...
const APP_ICO_TARGET = path.join(ROOT, "desktop", "src", "icon.ico");
...
function writePng(target, source, size) {
  fs.writeFileSync(target, renderLayer(source, size));
  console.log(`Generated ${path.relative(ROOT, target)} (${size} px)`);
}
...
function main() {
  ...
  writeIco(APP_ICO_TARGET, source);
  writeIco(path.join(ASSETS_DIR, "tray.ico"), source);
  writeIco(path.join(ASSETS_DIR, "tray-dev.ico"), source);
  writePng(path.join(ASSETS_DIR, "tray-template.png"), source, 16);
  writePng(path.join(ASSETS_DIR, "tray-template@2x.png"), source, 32);
  writePng(path.join(ASSETS_DIR, "tray-dev-template.png"), source, 16);
  writePng(path.join(ASSETS_DIR, "tray-dev-template@2x.png"), source, 32);
}

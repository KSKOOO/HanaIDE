...
2. 打开 HanaIDE → 设置 → 插件，把文件夹拖进安装区（或压缩成 .zip 拖入）
...
| Marketplace entry | 让插件出现在插件市场 | 写入你的 HanaIDE 插件目录条目 |
...
调试顺序：本地文件夹安装 → 设置页诊断 → 补 README/manifest → 需要公开时再写插件市场条目。
...
`minAppVersion`（可选）声明插件运行所需的最低 HanaIDE 版本。如果当前 app 版本低于该值，插件不会加载，状态标记为 `incompatible`。建议所有插件都声明此字段，避免用户在旧版本上遇到不兼容问题。
...
插件可以在顶部 tab 栏或编程工作台侧栏注册一个全页面视图。切换到该视图后，插件的 iframe 占据对应工作区空间。
...
设置 → 插件里的「打开插件市场」会进入独立的市场子页，该页面读取 `/api/plugins/marketplace`。HanaIDE 采用 Obsidian 式社区插件目录：第三方开发者把插件提交到维护者配置的插件目录，用户只浏览、安装、启用、禁用，不管理市场源。
...
没有配置环境变量时，HanaIDE 会先尝试读取 `${HANA_HOME}/plugin-marketplace/marketplace.json`（本地开发覆盖）。公开分发时建议通过 `HANA_PLUGIN_MARKETPLACE_URL` 指向你维护的插件目录。市场 index 的基本形状如下：

...
function getIpcMain() {
  ...
  const resolvedIpcMain = electron?.ipcMain || electron?.default?.ipcMain;
  if (!resolvedIpcMain) {
    throw new Error('Electron ipcMain is unavailable in this process');
  }
  return resolvedIpcMain;
}

function __setIpcMainForTests(ipcMain) {
...
module.exports = { wrapIpcHandler, wrapIpcBestEffortHandler, wrapIpcOn, __setIpcMainForTests };

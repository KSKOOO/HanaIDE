...
describe("Windows icon contract", () => {
  it("uses the app ICO for Windows packaging and installer surfaces", () => {
    ...
    expect(pkg.build.appId).toBe("com.hanaide.app");
  ...
  it("separates app window icon from tray icon and sets Windows taskbar identity", () => {
    ...
    expect(main).toContain("com.hanaide.app");
    expect(main).not.toContain("com.hanako.app");
  ...
  it("generates HanaIDE tray ICO files for normal and dev tray surfaces", () => {
    for (const fileName of ["tray.ico", "tray-dev.ico"]) {
      const entries = readIcoEntries(path.join(ROOT, "desktop", "src", "assets", fileName));
      const sizes = entries.map((entry) => entry.width).sort((a, b) => b - a);

...
describe('SettingsContent title placement', () => {
  ...
  it('notifies the modal shell when the active settings tab changes', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'settings.tabs.providers' }));
  ...
  it('does not render removed settings tabs and redirects stale tabs to coding-safe settings', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'settings.tabs.experiments' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'settings.tabs.bridge' })).not.toBeInTheDocument();
    expect(mockState.set).toHaveBeenCalledWith({ activeTab: 'general' });
  });

  it('normalizes stale bridge settings links to the general coding settings page', async () => {
    mockState.activeTab = 'bridge';
    const { SettingsContent } = await import('../../settings/SettingsContent');
    render(<SettingsContent variant="window" />);

    expect(screen.getByText('general tab')).toBeInTheDocument();
    expect(screen.queryByText('bridge tab')).not.toBeInTheDocument();
    expect(mockState.set).toHaveBeenCalledWith({ activeTab: 'general' });
  });

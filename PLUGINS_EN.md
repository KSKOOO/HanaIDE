...
2. Open HanaIDE → Settings → Plugins, drag the folder into the install area (or drag a .zip)
...
| Marketplace entry | Makes the plugin discoverable in the marketplace | Add an entry to your HanaIDE plugin catalog |
...
Debug order: install the local folder, inspect Settings diagnostics, finish README/manifest, then add a marketplace entry when the plugin is ready to publish.
...
`minAppVersion` (optional) declares the minimum HanaIDE version required to run the plugin. If the current app version is lower, the plugin will not load and its status is set to `incompatible`. All plugins should declare this field to prevent compatibility issues on older versions.
...
A plugin can register a full-page view in the top tab bar or coding workbench sidebar. When the user switches to that view, the plugin's iframe occupies the corresponding workspace area.
...
Plugins can register background tasks so HanaIDE can track and abort them. Runtime lifecycle is managed by `TaskRegistry`.
...
The "Open plugin marketplace" button in Settings -> Plugins opens a full marketplace subpage that reads `/api/plugins/marketplace`. HanaIDE follows the Obsidian-style community catalog model: third-party authors submit plugins to the catalog configured by the maintainer, while users browse, install, enable, and disable plugins without managing marketplace sources.

Default official catalog:

```text
https://example.com/hanaide-plugins/marketplace.json
```
...
Without either environment variable, HanaIDE first tries `${HANA_HOME}/plugin-marketplace/marketplace.json` for local development. For public distribution, set `HANA_PLUGIN_MARKETPLACE_URL` to the catalog you maintain. The marketplace index shape is:
...
```json
{
  ...
  "plugins": [{
    ...
    "distribution": {
      "kind": "release",
      "packageUrl": "https://example.com/hanaide-plugins/releases/download/demo-v1.0.0/demo.zip",
      "sha256": "..."
    },
    "versions": [
      {

...
function TreeNode({
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent) => {
    ...
    const openAction = () => {
      ...
      } else if (isWebRuntime() || !nativeRootDir) {
        previewFile();
      } else {
        window.platform?.openFile?.(path);
      }
    };
    const terminalTarget = deskWorkspaceMountId && !nativeRootDir
      ? (file.isDir ? subdir : (parent || '.'))
      : (file.isDir ? path : fullPath(nativeRootDir || deskBasePath, parent));
    const terminalCommand = deskWorkspaceMountId && !nativeRootDir
      ? `cd '${terminalTarget.replace(/'/g, "'\\''")}'`
      : (terminalTarget.includes('\\') || /^[A-Za-z]:[\\/]/.test(terminalTarget)
        ? `Set-Location -LiteralPath '${terminalTarget.replace(/'/g, "''")}'`
        : `cd '${terminalTarget.replace(/'/g, "'\\''")}'`);
    const hasPomXml = file.isDir
      && Array.isArray(treeFilesByPath[subdir])
      && treeFilesByPath[subdir].some(item => !item.isDir && item.name === 'pom.xml');
    const contextItems = buildWorkspaceContextMenuItems({
      t,
      target: {
        name: file.name,
        isDirectory: file.isDir,
        absolutePath: nativeRootDir ? path : null,
        relativePath: subdir,
        hasPomXml,
      },
      actions: {
        open: openAction,
        openWith: !file.isDir && nativeRootDir && window.platform?.openFile
          ? () => window.platform?.openFile?.(path)
          : undefined,
        revealInExplorer: nativeRootDir && window.platform?.showInFinder
          ? () => window.platform?.showInFinder?.(path)
          : undefined,
        openInTerminal: terminalTarget
          ? () => window.dispatchEvent(new CustomEvent('hana:coding-command', { detail: { action: 'terminal.run', command: terminalCommand } }))
          : undefined,
        maven: hasPomXml
          ? () => window.dispatchEvent(new CustomEvent('hana:coding-command', { detail: { action: 'terminal.run', command: `mvn -f '${terminalTarget.replace(/'/g, "'\\''")}/pom.xml' test` } }))
          : undefined,
        addToAssistant: () => {
          const store = useStore.getState();
          store.addAttachedFile?.({ path, name: file.name, isDirectory: file.isDir });
          store.requestInputFocus?.();
        },
        copy: () => navigator.clipboard.writeText(nativeRootDir ? path : subdir).catch(() => {}),
        copyPath: nativeRootDir ? () => navigator.clipboard.writeText(path).catch(() => {}) : undefined,
        copyRelativePath: () => navigator.clipboard.writeText(subdir).catch(() => {}),
        rename: actionEntries.length === 1 ? () => onBeginRename(subdir) : undefined,
        delete: actionEntries.length > 0 && (isWebRuntime() || deskWorkspaceMountId || window.platform?.trashItem)
          ? async () => {
    ...
    if (!file.isDir && screenshotSaveDir && isMarkdownFileName(file.name) && !isWebRuntime()) {
      const insertAt = Math.max(0, contextItems.findIndex(item => item.divider));
      contextItems.splice(insertAt, 0, {
    ...
    const renameItem = contextItems.find(item => item.label === t('workspace.context.rename'));
    if (renameItem) renameItem.label = t('desk.ctx.rename');
    const deleteItem = contextItems.find(item => item.label === t('workspace.context.delete'));
    if (deleteItem) deleteItem.label = deleteLabel;
    onShowMenu({
...
export function DeskTree({
  ...
  const handleBlankContextMenu = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    ...
    onShowMenu({
      ...
      items: [
        { label: t('desk.ctx.newMdFile'), action: () => { void onStartCreate('', 'markdown'); } },
        { label: t('desk.ctx.newFolder'), action: () => { void onStartCreate('', 'folder'); } },
      ],
    });

...
export class Agent {
  ...
  loadConfigOnly() {
    ...
    this.agentName = this._config.agent?.name || "HanaIDE";
  ...
  updateConfig(partial, options: any = {}) {
    ...
    // 更新身份
    const isZh = String(this._config.locale || "").startsWith("zh");
    if (partial.agent?.name) this.agentName = this._config.agent?.name || "HanaIDE";
  ...
  buildSystemPrompt( options: BuildSystemPromptOptions = {}) {
    ...
    const parts = [
      isZh
        ? "你运行在 HanaIDE 编程环境中（原名 OpenHanako），由 liliMozi 开发。"
        : "You are running in the HanaIDE coding environment (formerly OpenHanako), developed by liliMozi.",
    ];
    ...
    if (learnCfg.enabled && learnCfg.allow_github_fetch) {
      parts.push(isZh
        ? "\n## 主动技能获取\n\n" +
          ...
          "- 搜索：优先检查 HanaIDE 内置技能、当前工作区技能目录和可信文档；不要默认引导用户安装远程技能包\n" +
        ...
        : "\n## Proactive Skill Acquisition\n\n" +
          ...
          "- Search: prefer HanaIDE bundled skills, workspace skill directories, and trusted documentation; do not default to installing remote skill packages\n" +

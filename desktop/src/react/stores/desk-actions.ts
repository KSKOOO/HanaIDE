...
function normalizeSshRemotePath(value: string | null | undefined): string | null {
...
export interface SshStudioWorkspaceInput {
  host: string;
  port?: number;
  username: string;
  password?: string;
...
function normalizeStudioWorkspace(value: any): StudioWorkspace | null {
  ...
  return {
    ...
    remoteHost: typeof value?.remoteHost === 'string' ? value.remoteHost : null,
    remotePort: typeof value?.remotePort === 'number' ? value.remotePort : null,
    remoteUser: typeof value?.remoteUser === 'string' ? value.remoteUser : null,
...
export async function createSshStudioWorkspace(input: SshStudioWorkspaceInput): Promise<StudioWorkspace | null> {
  const host = input.host.trim();
  const username = input.username.trim();
  const remotePath = normalizeSshRemotePath(input.remotePath);
  const port = Number(input.port || 22);
  const s = useStore.getState();
  if (!host || !username || !remotePath || !Number.isInteger(port) || port < 1 || port > 65535 || !hasServerConnection(s)) {
    return null;
  }
  try {
    const res = await hanaFetch('/api/studio/workspaces', {
      ...
      body: JSON.stringify({
        ...
        label: input.label?.trim() || `${username}@${host}:${remotePath}`,
      }),
    });
    const data = await res.json();
    if (data.error) throw new Error(String(data.error));
    ...
    console.error('[workspace] create ssh studio workspace failed:', err);
    return null;
  }
}

export async function promptAndCreateSshStudioWorkspace(): Promise<StudioWorkspace | null> {
  const tr = window.t ?? ((p: string) => p);
  const host = window.prompt?.(tr('input.sshHostPrompt'))?.trim();
  if (!host) return null;
  const username = window.prompt?.(tr('input.sshUserPrompt'))?.trim();
  if (!username) return null;
  const portText = window.prompt?.(tr('input.sshPortPrompt'), '22')?.trim() || '22';
  const remotePath = window.prompt?.(tr('input.sshPathPrompt'), '/')?.trim() || '/';
  const label = window.prompt?.(tr('input.sshLabelPrompt'), `${username}@${host}:${remotePath}`)?.trim() || undefined;
  const password = window.prompt?.(tr('input.sshPasswordPrompt')) || undefined;

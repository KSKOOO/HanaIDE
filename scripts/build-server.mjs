...
const skipNftTrace = platform === "win32"
  || process.env.HANA_BUILD_SERVER_SKIP_NFT === "1"
  || process.env.HANA_BUILD_SERVER_SKIP_NFT === "true";
let fileList = null;
if (skipNftTrace) {
  const reason = platform === "win32" ? "win32 packaging" : "HANA_BUILD_SERVER_SKIP_NFT";
  console.warn(`[build-server] nft trace skipped (${reason}), keeping full runtime dependencies`);
} else {
